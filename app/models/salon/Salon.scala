...
//        SalonDAO.findOne(MongoDBObject("salonAccount.accountId" -> salonAccount.accountId,"salonAccount.password" -> salonAccount.password))
    ...
    def findSalonBySearchPara(searchParaForSalon : SearchParaForSalon) = {
        ...
        val region = if (searchParaForSalon.region.equals("all")) {
            "_id" -> MongoDBObject("$exists" -> true) 
        } else {
            "salonAddress.region" -> searchParaForSalon.region 
        }
        val salonIndustry = if (searchParaForSalon.salonIndustry.equals("all")) {
            "_id" -> MongoDBObject("$exists" -> true)
        } else {
            "salonIndustry" -> searchParaForSalon.salonIndustry
        }
        val salonName = if (searchParaForSalon.region.isEmpty) {
            "" $in "" 
        } else {
            "salonName" $in searchParaForSalon.salonName
        }
        ...
        salonList = dao.find($and(
            ...
            MongoDBObject("salonAddress.city" -> searchParaForSalon.city,region,salonIndustry,
                "seatNums" -> MongoDBObject("$lte" -> searchParaForSalon.seatNums.maxNum, "$gte" -> searchParaForSalon.seatNums.minNum)))).toList
        //以serviceType/haircutPrice作为check条件
        if (searchParaForSalon.serviceType.nonEmpty) {
            salonList.map { salon =>
                if(Service.findServiceTypeBySalonId(salon.id).containsSlice(searchParaForSalon.serviceType) 
