...
//    }    
    ...
    if(Position.findAll.isEmpty){
      Seq(
       ...
       Position(new ObjectId("531964e0d4d57d0a43771412"),"ChiefStylist"),	//首席技师	
       Position(new ObjectId("531964e0d4d57d0a43771413"),"Stylist"),		//技师
       Position(new ObjectId("531964e0d4d57d0a43771414"),"AdvancedAssistant"),	//高级助手
  ...
  def insertSampleData() {
   ...
   if(Stylist.findAll.isEmpty) { 
      Seq(
         ...
         Stylist(new ObjectId, new ObjectId("53202c29d4d5e3cd47efffd9"), 5, List(new IndustryAndPosition(new ObjectId,"AdvancedAssistant","Manicures")), List("sweet"), List("star"), List("Perm", "Dye", "Care"), List("male", "female"), List("1~10", "10~20", "20~30", "30~40"),"☆剪发：宁长勿短,宁厚勿薄,宁低勿高。 ","按照头皮通过样式建议☆脑袋矿泉根据（季节）的麻烦的建议,一个健康的状态样式建议☆日本四季可再生按照客户的后顾之忧,即使从后面的房子,改善☆（头按摩）","*（自主开发,本作的,运动员都特别喜欢）（印象系统,人类系统特别喜欢）阅读*电影DVD观看是一个（*易清洁,整齐*厨师,健康食物卡在）","建议适合女性的身材和喜好的麻烦,希望每一位客户,下一次不只是这一次,它建的故事,包括发型到下一个,但一☆砍你强​​调“易用性对待”,风格建议♪", (List(new OnUsePicture(new ObjectId, "logo", Some(1), None))), true, true ),
   ...
   if(User.findAll.isEmpty){
    ...
    if(Style.findAll.isEmpty) { 
      Seq(
        ...
        Style(new ObjectId("533151209aa6b4dfc54a1576"), "纹理披发", new ObjectId("53202c29d4d5e3cd47efffd9"), List(new OnUsePicture(new ObjectId, "main", Some(1), None),new OnUsePicture(new ObjectId, "side", Some(1), None),new OnUsePicture(new ObjectId, "back", Some(1), None)),"personality",List("Supple","Perm"),"super-short",List("brown","flax"),List("much"),List("silky","greasy","general"),List("bold","thin","moderate"),List("round-face","oval-face"),"此种发型清爽怡人,迎面而过,回眸一笑百媚生",List("one--twenty-five","twenty-five--thirty-five"),"male",List("star","evening-wear","others"),date("2014-03-12"),true),
        ...
        Style(new ObjectId("533151209aa6b4dfc54a1676"), "男士斜刘海长发", new ObjectId("53202c29d4d5e3cd47efffd3"), List(new OnUsePicture(new ObjectId, "main", Some(1), None),new OnUsePicture(new ObjectId, "side", Some(1), None),new OnUsePicture(new ObjectId, "back", Some(1), None)),"personality",List("Supple","Perm"),"super-short",List("brown","flax"),List("much"),List("silky","greasy","general"),List("bold","thin","moderate"),List("round-face","oval-face"),"此种发型清爽怡人,迎面而过,回眸一笑百媚生",List("one--twenty-five","twenty-five--thirty-five"),"male",List("star","evening-wear","others"),date("2014-03-12"),true),
